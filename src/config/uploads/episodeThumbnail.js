...
module.exports = {
  dest: path.resolve(__dirname, '..', '..', '..', 'public', 'images', 'episodes'),
  storage: multer.diskStorage({
    destination: (req, file, cb) => {
      cb(null, path.resolve(__dirname, '..', '..', '..', 'public', 'images', 'episodes'))
    },
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (invalidImage) {
      const error = {
        code: 415,
        message: 'Apenas miniaturas em png, jpeg e jpg são suportadas'
      }
